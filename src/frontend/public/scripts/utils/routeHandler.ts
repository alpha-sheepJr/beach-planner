...
import { beachKeyMap } from '../form/beachesMap.js';
...
export async function handleRouteChange(): Promise<void> {
  ...
  if (!match) return; // no matching route, do nothing
 ...
 */
function resolveToBeachIdNumber(beachIdString: string): number | undefined {
  ...
  return Beaches[key as keyof typeof Beaches];
}
