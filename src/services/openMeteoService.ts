...
> {
  ...
  timePeriod: { start: ISODateString; end: ISODateString };
  ...
  protected buildBaseParams(
    extra: Partial<Record<"hourly" | "daily", string[]>>
  ): Record<string, any> {
    return {
  ...
  abstract fetchHourly(): Promise<HourlyForecastData>;
  abstract fetchDaily(): Promise<DailyForecastData>;
}
...
class Weather extends GetForecast<HourlyWeatherRow, DailyWeatherRow> {
  async fetchHourly(): Promise<HourlyWeatherRow> {
  ...
  async fetchDaily(): Promise<DailyWeatherRow> {
    const params = this.buildBaseParams({
      daily: [
        "precipitation_sum",
...
class Marine extends GetForecast<HourlyMarineRow, DailyMarineRow> {
  async fetchHourly(): Promise<HourlyMarineRow> {
    const params = this.buildBaseParams({
      hourly: [
        "wave_height",
  ...
  async fetchDaily(): Promise<DailyMarineRow> {
    const params = this.buildBaseParams({
